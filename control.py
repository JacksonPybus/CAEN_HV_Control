            ...
            for channel in range(self.hv_control.get_channels_per_device(device_number)):
                ...
                for col_index, param in enumerate(headers[1:]):
                    ...
                    if param in ['VMon', 'IMon', 'ChStatus', 'Polarity']:  # These parameters are read-only
                        ...
                        options = ["Off", "On"]
                        pw_var = tk.StringVar(value="On" if param_value == 1 else "Off")
                        pw_dropdown = tk.OptionMenu(self.table_frame, pw_var, *options)
                        pw_dropdown.grid(row=row_index, column=col_index + 1)
                        
                        # Store the dropdown widget and its associated StringVar for later access
                        setattr(self, f"dropdown_{device_number}_{channel}_{param}", pw_dropdown)
                        setattr(self, f"dropdown_var_{device_number}_{channel}_{param}", pw_var)  # Store the StringVar
        ...
        for device_number in self.hv_control.device_numbers:
            for channel in range(self.hv_control.get_channels_per_device(device_number)):
                for param in ["VSet", "ISet", "MaxV", "RUp", "RDwn", "Trip", "PDwn", "Pw"]:
                    ...
                    dropdown_widget = getattr(self, f"dropdown_{device_number}_{channel}_{param}", None)
                    ...
                    elif dropdown_widget:
                        new_value = dropdown_widget.cget("text")  # Get the selected value ("On" or "Off")
            ...
            for channel in range(self.hv_control.get_channels_per_device(device_number)):
                for param in ["VSet", "ISet", "MaxV", "RUp", "RDwn", "Trip", "PDwn", "Pw"]:
                    ...
                    if entry_widget:
                        ...
                        prev_value = self.previous_values.get(device_number, {}).get(channel, {}).get(param, None)
                        if prev_value is not None:
                            # For the dropdown, we update the associated StringVar
                            dropdown_var = getattr(self, f"dropdown_var_{device_number}_{channel}_{param}", None)
                            if dropdown_var:
                                dropdown_var.set("On" if prev_value == 1 else "Off")  # Set the value of the StringVar
